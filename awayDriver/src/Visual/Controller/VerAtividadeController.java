...
public class VerAtividadeController implements Initializable{
    ...
    public void initialize(URL url, ResourceBundle rb) { 
        ...
        ArrayList<String> questoes = ((AtividadeQuiz) tarefa.getAtividade()).getQuestoes();
        ArrayList<Boolean> respostas = ((RespostaQuiz) tarefa.getResposta()).getRespostas();
        
        for (int i = 0; i < questoes.size(); i++) {
            JFXTextField enunciado = new JFXTextField(questoes.get(i));
            JFXCheckBox resposta = new JFXCheckBox();
            if (respostas.get(i) == true){
                resposta.selectedProperty().setValue(Boolean.TRUE);
            } else {
                resposta.selectedProperty().setValue(Boolean.FALSE);
            }
